...
import { Home, MessageSquare, Bell, Megaphone, User as UserIcon } from 'lucide-react';
...
export function BottomNavigation({
  ...
  setUnreadConvCount: (count: number) => void;
  loadConversations: () => void;
  hasNewUnreadSOS: boolean;
  setHasNewUnreadSOS: (has: boolean) => void;
  ownerProfile: any;
}) {
  if (!user || view === 'finder' || view === 'install_pwa') return null;
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-100 px-2 py-3 flex justify-around items-center z-50 shadow-[0_-4px_20px_rgba(0,0,0,0.05)]">
      ...
      {/* Alertas — botão principal (FAB central) */}
      <button
        onClick={() => {
          setView('lost_pets');
      ...
      >
        <div className={`w-[60px] h-[60px] rounded-full flex items-center justify-center shadow-lg border-4 border-white active:scale-90 transition-transform ${hasNewUnreadSOS && view !== 'lost_pets' ? 'bg-red-500 shadow-red-200 animate-pulse' : 'bg-orange-500 shadow-orange-200'}`}>
          <Megaphone className="w-7 h-7 text-white" />
        </div>
        <span translate="no" className="text-[11px] font-bold uppercase text-orange-500 mt-1">Alertas</span>
        {hasNewUnreadSOS && view !== 'lost_pets' && (
          <span className="absolute top-0 right-1/2 translate-x-7 w-3 h-3 bg-red-500 rounded-full border-2 border-white" />
        )}
      </button>
